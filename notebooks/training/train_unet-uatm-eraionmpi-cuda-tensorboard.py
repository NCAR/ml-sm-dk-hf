...
# instrument training with tensorboard
...
#mnvars = 8*4
mnvars = 8*1
md = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/U850.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv2 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/V850.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv3 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/Q850.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv4 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/T700.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv5 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/Z700.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv6 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/Z500.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv7 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/U250.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
mdv8 = np.load('/glade/scratch/dkorytin/erai-on-mpigrid/V250.ERAI.MPIGRID.1979-2018.npy')[0:mndays*1]
...
for ii in range(mndays):
    ...
    #for iii in range(4):
       rowdd.append( cv2.resize(md[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv2[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv3[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv4[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv5[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv6[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv7[ii*1+iii][0],(16,16)) )
       rowdd.append( cv2.resize(mdv8[ii*1+iii][0],(16,16)) )
       dd.append(rowdd)
...
class GenerativeNet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        #self.linear = torch.nn.Linear(ires**2*mnvars, ires**2*2048)
...
#         )
        ...
        self.conv2 = nn.Sequential(
            nn.ConvTranspose2d(
                in_channels=1024+1024, out_channels=512, kernel_size=4,
                stride=2, padding=1, bias=True
            ),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True)
        ...
        # smoothing 5x5 stride-1 deconv
        self.conv6 = nn.Sequential(
            nn.ConvTranspose2d(
                in_channels=64, out_channels=nvars, kernel_size=5,
...
#    error.backward()
    ...
    img_loss = mse(generated, y) #+ sserr
    img_loss.backward()
...
## load state
#state = torch.load("/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_state")
...
#for epoch in range(loaded_epoch, num_epochs):
    ...
    for n_batch in range(0, len(osamples)-mb+1-1, mb):  # mb-sized sample start indices 
        ...
        if (n_batch) % 20000 == 0:
            ...
            ######torch.save(state, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_state")
            ...
            #torch.save(gstate, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_generator-e"+str(epoch))
            if epoch % 50 == 0 and epoch>0:
                #torch.save(gstate, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_generator-10s-e"+str(epoch))
                torch.save(state, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_state")
                torch.save(gstate, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_generator-e"+str(epoch))
            ...
            if os_err < 0.003:
                print("SAVING GENERATOR", os_err)
                torch.save(gstate, "/glade/scratch/dkorytin/saved_models/uatm-eraionmpi-cuda_generator-e"+str(epoch))

            # add training result to tensorboard
            writer.add_scalars('uatm-nocos', {'os_err':float(os_err),'g_err':float(g_error)}, epoch)
